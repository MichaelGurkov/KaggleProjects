...
train %>%
  ...
  geom_col(width = 0.5, position = "dodge") + 
  scale_fill_viridis_d(alpha = 0.8,option = "inferno") + 
  ggtitle("Titanic survival by pclass") + ylab(NULL) + xlab(NULL)
...
train %>%
  ...
  group_by(sex, pclass) %>% 
  mutate(prop = n[survived == "yes"] / sum(n)) %>% 
  filter(survived == "yes") %>% 
  ggplot(aes(x = pclass, y = sex, size = prop, fill = prop)) + 
  geom_point()
